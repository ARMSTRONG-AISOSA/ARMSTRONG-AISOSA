# 👋 Hi, I'm ARMSTRONG OMOREGIE

### Full-Stack Web Developer/Software Engineer

I’m a **Full-Stack Web Developer** passionate about building modern, responsive, and user-focused web applications.

I enjoy turning ideas and designs into functional digital products, working across the frontend and backend, designing databases, integrating APIs, and continuously learning new technologies.

I’m also open to **collaborating on interesting projects, contributing to open source, and building useful products.**

---

## 🧑‍💻 About Me

* 💻 Full-Stack Web Developer
* 🌐 Passionate about building modern web applications
* 🎨 Interested in UI/UX and translating designs into functional interfaces
* 🗄️ Experienced with relational and NoSQL databases
* 🔌 Experienced with RESTful API integration and backend development
* 🐳 Interested in containerization and modern development workflows
* 🦀 Currently expanding my knowledge of Rust
* 🤝 Open to collaboration and interesting projects

---

## 🛠️ Tech Stack

### 💬 Languages

[![Languages](https://skillicons.dev/icons?i=html,css,js,ts,c,rust)](https://skillicons.dev)

### 🎨 Frontend Development

[![Frontend](https://skillicons.dev/icons?i=react,nextjs,tailwind,materialui,bootstrap,sass,vite)](https://skillicons.dev)

### ⚙️ Backend Development

[![Backend](https://skillicons.dev/icons?i=nodejs,express)](https://skillicons.dev)

### 🗄️ Databases & Backend Services

[![Databases](https://skillicons.dev/icons?i=postgresql,mongodb,supabase,firebase)](https://skillicons.dev)

### 🔧 Tools & DevOps

[![Tools](https://skillicons.dev/icons?i=git,github,docker,linux,npm,postman,vscode)](https://skillicons.dev)

### 🎨 Design & Productivity

[![Design](https://skillicons.dev/icons?i=figma,notion)](https://skillicons.dev)

### 🌐 CMS

[![CMS](https://skillicons.dev/icons?i=wordpress)](https://skillicons.dev)

---

## 🚀 What I Build

I enjoy working on projects involving:

* 🌐 Full-stack web applications
* 📊 SaaS platforms and dashboards
* ⚛️ Modern React and Next.js applications
* 🔌 RESTful APIs and third-party integrations
* 🗄️ Database-driven applications
* 🎨 Responsive interfaces and design systems
* 🤖 AI-powered applications and tools
* 🏢 Business and productivity solutions

---

## 📌 Featured Projects

### 🤖 FederateAI

An AI-powered knowledge platform designed to bring fragmented business knowledge together and make it easier to search and access.

**Technologies:** Next.js, TypeScript, Tailwind CSS, Supabase, Ahnlich, Docker

---

### 💼 Developer Portfolio

My personal developer portfolio showcasing my skills, projects, experience, and technical interests.

🌐 **Live Website:**
https://bossstrong-dev.vercel.app/

**Technologies:** Nextjs, TypeScript, Tailwind CSS, ShadCn, Supabase, Resend

---

## 🌱 Currently Learning & Exploring

I'm continuously expanding my knowledge across the full development stack.

Currently exploring:

* 🦀 Rust
* 🐘 Advanced PostgreSQL
* 🔗 Backend architecture
* 🐳 Docker & containerized applications
* 🧪 Software testing
* ☁️ Modern deployment workflows
* 🎨 UI/UX design for SaaS applications

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=BossStrung&show_icons=true&theme=tokyonight&hide_border=true" alt="Armstrong's GitHub Stats" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=BossStrung&theme=tokyonight&hide_border=true" alt="Armstrong's GitHub Streak" />
</p>

---

## 📈 Most Used Languages

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=BossStrung&layout=compact&theme=tokyonight&hide_border=true" alt="Armstrong's Most Used Languages" />
</p>

---

## 🤝 Let's Connect

I'm always interested in connecting with other developers, collaborating on projects, discussing ideas, and exploring opportunities to build useful products.

* 🌐 **Portfolio:** https://bossstrong-dev.vercel.app/
* 📝 **Blog:** https://bossstrong-dev.vercel.app/blog/
* 💼 **LinkedIn:** https://www.linkedin.com/in/armstrong-omoregie-02367a271/
* 🐦 **Twitter/X:** https://twitter.com/BossStrung
* 📧 **Email:** [ammycan6@gmail.com](mailto:ammycan6@gmail.com)

---

### 💡 "Build. Learn. Improve. Repeat."

Thanks for stopping by my profile! 👋

⭐ Feel free to explore my repositories and connect with me.
